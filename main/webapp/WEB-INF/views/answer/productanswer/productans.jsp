...
<body>
			...
			<div class="card-body">
				<form>
					...
					<a href="<c:url value="/answers" />" class="btn btn-outline-secondary">목록</a> <a
						href="<c:url value="/productanswers/productupdate?productUpdateAn=${answerlistone.productid}"/>" class="btn btn-outline-warning">수정</a>
					<a href="<c:url value="javascript:deleteConfirm('${answerlistone.productid }')"/>" class="btn btn-outline-danger">삭제</a>
				</form>
			</div>
		</div>
	</div> 
	...
	{
		if(confirm("삭제하시겠습니까?")==true) location.href = "./productDelete?answerJs=" +answerJs;
		else return;	
	}
	</script>
</body>
